...
from core.motion_analyzer import MotionAnalyzer  # 假设你放在了 core 文件夹
class StreamController:
    ...
    def __init__(self, 
        ...
        # 2. 生成当前UI会话的时间戳文件名（格式：20260106_153025_conversation.txt）
        self.session_timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        #self.log_file_path = os.path.join(self.log_dir, f"{self.session_timestamp}_conversation.txt")
        self.log_file_path = os.path.join(self.log_dir,"reply.txt")
        # 3. 临时存储当前轮次的用户文本（用于和AI响应配对）
        self.current_user_text = ""
        #print(f"📝 当前会话日志文件: {self.log_file_path}")
        # =============================================
    ...
    def _save_conversation_to_file(self, user_text: str, ai_response: str):
        """
        将用户文本和AI响应保存到本地TXT文件
        :param user_text: 用户语音识别的文本
        :param ai_response: AI的回复文本
        """
        try:
            # 格式化当前时间（用于日志内的时间标注）
            current_time = datetime.now().strftime("%Y-%m-%d %H:%M")
            # 构建对话内容（清晰分隔用户/AI，方便阅读）
...
"""
    ...
    def _on_ai_response_received(self, success: bool, response: str = None, error: str = None):
        ...
        if success and response:
            ...
            # ========== 新增：调用保存方法，写入用户文本+AI响应 ==========
            self._save_conversation_to_file(self.current_user_text, response)
            # =============================================================
            # 使用 daemon=True 守护线程，防止主程序退出时线程卡死
            # 必须用线程，因为 OpenAI 的 API 请求会消耗时间，不用线程会卡住语音播报
            analysis_thread = threading.Thread(
                target=self.motion_analyzer.analyze_text, 
                args=(response,),
                daemon=True
            )
